...
from datasets.dataset_aus import AusDataset
...
class Coach:
    def __init__(self, opts):
        ...
        self.aus_loss = torch.nn.MSELoss(reduce=False).eval()
        self.predictor = Predict().to(self.device)
        self.load_predictor()
        ...
        #                                   drop_last=True)
        train_dataset = AusDataset(opts, 'Train')
        ...
        test_dataset = AusDataset(opts, 'Test')
    ...
    def load_predictor(self):
        ckpt_predictor = torch.load(model_paths['predictor'])
        self.predictor.load_state_dict(ckpt_predictor)

    def aus_parse_batch(self, batch):
        x, y, y_hat, latents = None, None, None, None
        x = (batch['real_img'].float()).to(self.device)
        y = (batch['desired_img'].float()).to(self.device)
    #    desired_img = (batch['desired_img'].float()).to(self.device)
        real_cond = (batch['real_cond'].float()).to(self.device)
        desired_cond = (batch['desired_cond'].float()).to(self.device)
        delta_au = (desired_cond - real_cond).unsqueeze(1)
        return x, y, y_hat, latents, delta_au, desired_cond
    ...
    def perform_forward_on_batch(self, batch, train=False):
        ...
        x, y, y_hat, codes, delta_au, desired_cond = self.aus_parse_batch(batch)
     #   print(delta_au.size())
        '''
        print(delta_au)
        '''
    ...
    def calc_loss(self, x, y, y_hat, desired_cond, latent, weights_deltas):
        loss_dict = {}
        loss = 0.0
        id_logs = None
        ...
        if self.opts.id_lambda > 0:
            loss_id, sim_improvement, id_logs = self.id_loss(y_hat_resized, x_resized, x_resized)
        ...
        if self.opts.lpips_lambda > 0:
            loss_lpips = self.lpips_loss(y_hat_resized, x_resized)#y_resized#
        ...
        loss_au = (self.aus_loss(self.predictor(y_hat), desired_cond)).mean()
        loss_dict['loss_aus'] = float(loss_au)
        loss += 25 * loss_au
