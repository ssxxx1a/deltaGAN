...
class HyperStyle(nn.Module):
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            ...
            e4e_checkpoint_path = model_paths['e4e_w_encoder']  # 实际上不只包含encoder
            ckpt = torch.load(e4e_checkpoint_path)  # 'map_location='cpu'
            # self.__load_latent_avg(ckpt, repeat=self.n_styles)
            self.latent_avg = None
            from models.my_encoder.psp_encoders import Encoder4Editing
            if self.opts.load_w_encoder:
                self.w_encoder = Encoder4Editing(50, 'ir_se', None)
                self.w_encoder.load_state_dict(self.get_keys(ckpt, 'encoder'), strict=True)
            self.decoder.load_state_dict(self.get_keys(ckpt, 'decoder'), strict=False)
            #

    def forward(self, x, resize=True, input_code=False, randomize_noise=True, return_latents=False,
                return_weight_deltas_and_codes=False, weights_deltas=None, y_hat=None, codes=None, aus=None):
        ...
        if input_code:
            ...
            hypernet_outputs = self.hypernet(x_input, aus)
